  ...
  <div class="highcharts-figure">
    ...
    <div class="insight-grid">
      <div class="insight-card redred">
        <span class="insight-label">Активно невовлечённых</span>
        <span class="insight-value">Женщины {{ activelyDisengaged[0] }}% · Мужчины {{ activelyDisengaged[1] }}%</span>
        <span class="insight-meta">Требуют первоочередного внимания</span>
      </div>

      <div class="insight-card green">
        <span class="insight-label">Высоко вовлечённых</span>
        <span class="insight-value">Женщины {{ highlyEngaged[0] }}% · Мужчины {{ highlyEngaged[1] }}%</span>
        <span class="insight-meta">Ядро вовлечённой аудитории</span>
      </div>

      <div class="insight-card yellow">
        <span class="insight-label">Нейтральные</span>
        <span class="insight-value">Женщины {{ neutral[0] }}% · Мужчины {{ neutral[1] }}%</span>
        <span class="insight-meta">Потенциал для перевода в вовлечённых</span>
      </div>
    </div>

    <p class="highcharts-description">
      Pictorial-диаграмма распределяет сотрудников по группам вовлечённости отдельно для
      женщин и мужчин на основе GetMetricsRespondentGroups. Каждый силуэт заполняется
      пятью уровнями: высоко вовлечённые, вовлечённые, нейтральные, невовлечённые и
      активно невовлечённые. Сумма по каждому силуэту составляет 100%.
    </p>
...
const ensureHighchartsModules = async () => {
  ...
  const [PictorialModule, ExportingModule, AccessibilityModule] = await Promise.all([
    import('highcharts/modules/pictorial.js'),
    import('highcharts/modules/exporting.js'),
    import('highcharts/modules/accessibility.js')
  ])

  initHighchartsModule(PictorialModule)
...
const chartOptions = computed(() => ({
  ...
  title: {
    text: 'Группы вовлечённости по полу',
    style: { color: '#000000', fontSize: '18px', fontWeight: '700' }
  },
  subtitle: {
    text: 'Каждый силуэт = 100% сотрудников этого пола, разбитых по уровням вовлечённости',
    style: { color: '#444444', fontSize: '12px' }
  },
  accessibility: {
    enabled: true,
    description: 'Pictorial-диаграмма распределения групп вовлечённости по полу.',
    screenReaderSection: {
      beforeChartFormat:
  ...
  plotOptions: {
    series: {
      ...
      dataLabels: {
        ...
        align: 'center',
        format: '{y}%',
        style: { color: '#000000', fontSize: '11px', fontWeight: '600', textOutline: 'none' }
      },
      stacking: 'normal',
      paths: [
        { definition: womanPath },
        { definition: manPath }
      ]
    }
  },
  series: [
    ...
    {
      name: 'Активно невовлечённые',
      data: activelyDisengaged,
      color: '#f7a8a1'
    }
  ],
  responsive: {
...
const initChart = async () => {
  ...
  try {
    ...
    console.error('Error rendering pictorial chart:', error)
